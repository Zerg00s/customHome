    ...
    function sp($q, $http, $log) {
        ...
        sp.getFormDigest = function () {
            ...
            if (sp.formDigest) {
                ...
                var settings = Request.post(_spPageContextInfo.webAbsoluteUrl + "/_api/contextinfo").settings;
        ...
        function returnStorageValue(response){
            if (response.data.d.results.length > 0) {
                return parseJsonValue(response.data.d.results[0].Value);
            }
            else {
                return parseJsonValue(response.data.d.Value);
            }
        }
        ...
        function UserStorage(listTitle) {
                ...
                return this.get(key).then(function (items) {
                    ...
                    return $q.all([formDigestPromise, listEntityNamePromise]).then(function (values) {
                        ...
                        else if(items.Value)
                        {
                            ...
                            return updateItemConditional(values, that.listTitle, key, value, items);
                        }
                        else {
            ...
            this.get = function (key) {
                ...
                var settings = Request.get(url).settings;
                return $http(settings).then(returnListItem).catch(errorHandler)
                //return $http(settings).then(returnStorageValue).catch(errorHandler)
            }
        ...
        sp.getUserStorage = function (listTitle, useRootSite) {
            if (useRootSite){
                urls.lists = useRootSite + "/_api/web/lists";;
            }
